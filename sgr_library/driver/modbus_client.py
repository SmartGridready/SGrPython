...
class SGrModbusClient:
    ...
    async def value_decoder(
        ...
        try:
            ...
            logging.exception(
                f"Timeout reading register at address {addr} with slave ID {slave_id}"
            )
            return None
    ...
    async def value_encoder(
        ...
        try:
            ...
            logging.exception(
                f"Timeout writing value to register at address {addr} with slave ID {slave_id}"
            )
        except ValueError as e:
            logging.exception(f"Value error occurred: {e}")
        except Exception as e:
            logging.exception(
                f"An unexpected error occurred while writing value to register: {e}"
            )
