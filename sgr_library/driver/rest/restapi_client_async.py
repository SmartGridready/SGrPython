...
class RestDataPoint(DataPointProtocol):
    ...
    async def read(self):
        return await self._interface.get_val(
            self._method, self._path, self._header, self._body, self._query
        )
...
class SgrRestInterface(BaseSGrInterface):
    ...
    def __init__(
        ...
        if (
            ...
            raw_interface = self._root.interface_list.rest_api_interface
        else:
            raise Exception("Invalid")
        raw_fps = []
        if (
            raw_interface.functional_profile_list
            and raw_interface.functional_profile_list.functional_profile_list_element
        ):
            raw_fps = raw_interface.functional_profile_list.functional_profile_list_element

        fps = [RestFunctionProfile(profile, self) for profile in raw_fps]
        self._function_profiles = {fp.name(): fp for fp in fps}

        try:
            description = raw_interface.rest_api_interface_description
            description.rest_api_authentication_method
            request_body = str(
